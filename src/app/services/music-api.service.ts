...
export class MusicApiService {
    ...
    // --- GAME METHODS ---
    getGameFamous(category: 'singers' | 'soccer' | 'movies' | 'random'): Observable<string[]> {
        const fallbacks: Record<string, string[]> = {
            singers: ['Bad Bunny', 'Karol G', 'Shakira', 'J Balvin', 'Maluma', 'Daddy Yankee', 'Ozuna', 'Anuel AA', 'Rauw Alejandro', 'Feid', 'Taylor Swift', 'Drake', 'The Weeknd', 'Dua Lipa', 'Ed Sheeran'],
            soccer: ['Lionel Messi', 'Cristiano Ronaldo', 'Neymar Jr', 'Kylian Mbappé', 'Erling Haaland', 'Vinicius Jr', 'Luka Modric', 'Kevin De Bruyne', 'Robert Lewandowski', 'Mohamed Salah', 'James Rodriguez', 'Luis Diaz'],
            movies: ['Spider-Man', 'Batman', 'Iron Man', 'Joker', 'Harry Potter', 'Darth Vader', 'Jack Sparrow', 'Thanos', 'Wolverine', 'Thor', 'Wonder Woman', 'Black Panther', 'Shrek', 'Elsa'],
            random: ['Bad Bunny', 'Messi', 'Spider-Man', 'Shakira', 'Cristiano Ronaldo', 'Batman', 'Taylor Swift', 'Neymar', 'Iron Man', 'Drake', 'Mbappé', 'Joker']
        };

        if (category === 'singers') {
            // Fetch top artists from iTunes
            const url = `https://itunes.apple.com/us/rss/topsongs/limit=50/json`;
            return this.http.get<any>(url).pipe(
                map(res => {
                    const entries = res.feed?.entry;
                    if (!entries || entries.length === 0) return fallbacks['singers'];
                    const artists = entries.map((e: any) => e['im:artist']?.label || '').filter((a: string) => a.length > 0);
                    const unique = [...new Set(artists)] as string[];
                    return unique.length >= 10 ? unique.slice(0, 20) : fallbacks['singers'];
                }),
                catchError(() => of(fallbacks['singers']))
            );
        }

        // For other categories, use Datamuse or fallbacks
        if (category === 'soccer' || category === 'movies') {
            return of(fallbacks[category]);
        }

        // Random: mix all categories
        const mixed = [...fallbacks['singers'].slice(0, 5), ...fallbacks['soccer'].slice(0, 5), ...fallbacks['movies'].slice(0, 5)];
        return of(mixed.sort(() => Math.random() - 0.5));
    }

    getGameWord(): Observable<string> {
        const fallbackWords = [
            'Pizza', 'Playa', 'Guitarra', 'Montaña', 'Café', 'Concierto', 'Avión', 'Teléfono',
            'Navidad', 'Cumpleaños', 'Hospital', 'Aeropuerto', 'Universidad', 'Supermercado',
            'Restaurante', 'Biblioteca', 'Discoteca', 'Estadio', 'Gimnasio', 'Parque',
            'Helado', 'Chocolate', 'Hamburguesa', 'Tacos', 'Sushi', 'Pasta', 'Ensalada',
            'Perro', 'Gato', 'Elefante', 'León', 'Delfín', 'Mariposa', 'Araña',
            'Fútbol', 'Baloncesto', 'Tenis', 'Natación', 'Boxeo', 'Ciclismo',
            'Cine', 'Teatro', 'Museo', 'Zoológico', 'Acuario', 'Circo'
        ];

        // Try to get a random word from Datamuse API
        const randomTopic = ['food', 'animal', 'sport', 'place', 'music'][Math.floor(Math.random() * 5)];
        const url = `https://api.datamuse.com/words?topics=${randomTopic}&max=20`;

        return this.http.get<any[]>(url).pipe(
            map(words => {
                if (words && words.length > 0) {
                    const filtered = words.filter((w: any) => w.word && w.word.length >= 4 && w.word.length <= 15);
                    if (filtered.length > 0) {
                        const word = filtered[Math.floor(Math.random() * filtered.length)].word;
                        return word.charAt(0).toUpperCase() + word.slice(1);
                    }
                }
                return fallbackWords[Math.floor(Math.random() * fallbackWords.length)];
            }),
            catchError(() => of(fallbackWords[Math.floor(Math.random() * fallbackWords.length)]))
        );
    }
}
